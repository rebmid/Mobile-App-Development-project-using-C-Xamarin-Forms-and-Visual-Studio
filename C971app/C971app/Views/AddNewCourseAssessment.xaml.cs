            ...
            try
            {
                if (RequiredAssessmentFieldPopulated())
                {
                    ...
                    using (SQLiteConnection connection = new SQLiteConnection(App.FilePath))
                    {
                        ...
                        {
                            string warningMessage = $"You can't add more than one {assessment.Type} Assessment Type per Course.";
                ...
                else
                {
                    ...
                    {
                        throw new ApplicationException("Assessment Name REQUIRED.");
                    }

                    if (PickerAssessmentType.SelectedItem == null)
                    {
                        throw new ApplicationException("Assessment Type REQUIRED.");
                    }

                    if (String.IsNullOrWhiteSpace(DatePickerStartDate.Date.ToString()))
                    {
                        throw new ApplicationException("Start Date REQUIRED.");
                    }

                    if (String.IsNullOrWhiteSpace(DatePickerEndDate.Date.ToString()))
                    {
                        throw new ApplicationException("End Date REQUIRED.");
                    }
                }
            }
